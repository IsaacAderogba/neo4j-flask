...
class User:
    ...
    def add_post(self, title, tags, text):
        ...
        post = Node("Post", id=str(uuid.uuid4()), title=title,
                    text=text, timestamp=int(datetime.now().strftime("%s")),
    ...
    def commonality_of_user(self, user):
        ...
        likes = graph.run(query1, username1=self.username,
                          username2=user.username).data()
        print(likes)
        ...
        tags = graph.run(query2, username1=self.username,
                         username2=user.username).data()
        print(tags)
...
def todays_recent_posts(n):
  ...
  """
    ...
    res = graph.run(query, today=today, n=n).data()
    return res
